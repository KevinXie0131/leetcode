...
public class Q473_Matchsticks_to_Square {
    ...
    public static boolean makesquare(int[] matchsticks) {
        ...
        //先排序
        Arrays.sort(matchsticks); // less efficient
       ...
       /* Arrays.sort(matchsticks); // works too and more efficient
    ...
    public static boolean backtracking(int[] matchsticks, int index, int edge[], int edgeLen){
        if(index == matchsticks.length){
         //   return true; // works too
            if(edge[0] == edgeLen && edge[1] == edgeLen && edge[2] == edgeLen && edge[3] == edgeLen){// can be commented
                return true;
