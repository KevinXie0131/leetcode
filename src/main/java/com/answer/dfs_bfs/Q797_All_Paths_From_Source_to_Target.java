...
public class Q797_All_Paths_From_Source_to_Target {
    ...
    public void dfs1(int[][] graph, int x, int n,  List<List<Integer>> result, List<Integer> path ) {
        if(x == n){ //如果遍历到最后一个节点，则停止遍历
            // 使用new ArrayList<>(path)创建path的一个副本，以避免后续对path的修改影响已经添加的结果
            result.add(new ArrayList<>(path));    //达到目标节点，保存此条路径并结束搜索
            return;
        }
        //遍历当前节点所有关联的节点
        for(int y : graph[x]){
            ...
            path.remove(path.size() - 1);  //回溯(ArrayList的remove()方法传入整数会被作为下标，使用path.size()-1可以严格控制删除最后一个元素)
        }
    }
    /**
     * 记忆化递归
