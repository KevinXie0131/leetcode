...
public class Q827_Making_A_Large_Island { // Hard 困难
    ...
    public int largestIsland(int[][] grid) { // 标记岛屿(对不同的岛屿进行编号) + 合并
        ...
        boolean[][] visited = new boolean[m][n];// 定义二位boolean数组记录该位置是否被访问
        HashMap<Integer, Integer> getSize = new HashMap<>(); // 定义一个HashMap，记录某片岛屿的标记号和面积
        HashSet<Integer> set = new HashSet<>(); // 定义一个HashSet，用来判断某一位置水四周是否存在不同标记编号的岛屿
        // 定义一个boolean变量，看看DFS之后，是否全是岛屿
        boolean isAllIsland = true;  // 标记是否整个地图都是陆地

        for (int i = 0; i < m; i++) {  // 遍历二维数组进行DFS搜索，标记每片岛屿的编号，记录对应的面积
            for (int j = 0; j < n; j++) {
                if (grid[i][j] == 0) {
                    isAllIsland = false;
                }
    ...
    public void dfs_visited(int[][] grid, int x, int y, boolean[][] visited) {
        ...
        dfs_visited(grid, x, y + 1, visited);  // 继续向下层搜索
