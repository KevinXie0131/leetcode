...
public class Q133_Clone_Graph_1 {
    ...
    public Node cloneGraph(Node node) {
        ...
        if (visited.containsKey(node)) {  // 如果该节点已经被访问过了，则直接从哈希表中取出对应的克隆节点返回
            return visited.get(node);
        }
        Node cloneNode = new Node(node.value, new ArrayList());// 克隆节点，注意到为了深拷贝我们不会克隆它的邻居的列表
    ...
    public Node cloneGraph1(Node node) {
        ...
        LinkedList<Node> queue = new LinkedList<> (); // 将题目给定的节点添加到队列
        queue.offer(node);
    ...
    public Node cloneGraph4(Node node) {
        ...
        while(!queue.isEmpty()){
            Node cur = queue.poll();     // 从队列中获取一个待处理的节点
            Node cloneNode = cloneNodes[cur.value];   // 待处理的节点一定是克隆好了的，直接获取其克隆节点
            for(Node neighbor: cur.neighbors){
                if(cloneNodes[neighbor.value] == null){  // 处理当前节点的邻接节点
