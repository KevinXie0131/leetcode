...
public class ShortestPathBellmanFordSingleSourceSPFA {
    /**
    ...
    public static void main(String[] args) {
        ...
        List<List<Edge6>> graph = new ArrayList<>(); // 构建邻接表
        for (int i = 0; i <= n; i++) {
        ...
        while (m-- > 0) {
            ...
            graph.get(u).add(new Edge6(u, v, weight));
        }
        ...
        // 调用算法
        int[] minDist = ShortestPathBellmanFordSingleSourceSPFA.spfa(n, graph, src, k);
        // 校验起点->终点
     ...
     */
    public static int[] spfa(int n, List<List<Edge6>> graph, int startIdx, int k) {
        // 定义最大范围
        ...
        while (k + 1 > 0 && !queue.isEmpty()) { // 限定松弛 k+1 次
            ...
            while (curSize-- > 0) { //分层控制，限定本次松弛只针对上一次松弛更新的节点，不对新增的节点做处理
                ...
                int cur = queue.poll();
                // 获取cur节点关联的边，进行松弛操作
                List<Edge6> relateEdges = graph.get(cur);
                for (Edge6 edge : relateEdges) {
...
class Edge6 {
    ...
    public Edge6(int u, int v) {
        this.u = u;
        this.v = v;
        this.val = 0;
    }

    public Edge6(int u, int v, int val) {
        this.u = u;
        this.v = v;
        this.val = val;
    }
