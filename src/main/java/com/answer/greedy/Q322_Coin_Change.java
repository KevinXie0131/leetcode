...
/*        int[] coins = {411,412,413,414,415,416,417,418,419,420,421,422};
    ...
    public int findWay(int[] coins, int remain){
        if(remain < 0) return -1;   // 金额小于0，无法兑换
        if(remain == 0) return 0;  // 目标金额为0，硬币数为0
        // 记忆化的处理，memo[n]用赋予了值，就不用继续下面的循环
        // 直接的返回memo[n] 的最优值
        if(memo[remain] != 0){
            return memo[remain]; // 已经计算过，直接返回
        }
        int min = Integer.MAX_VALUE;
        for(int coin : coins){
            int res = findWay(coins,remain - coin); // 递归计算剩余金额所需硬币数
            if(res >= 0 && res < min){
                min = res + 1; // 加1，是为了加上得到res结果的那个步骤中，兑换的一个硬币 // 当前硬币加上子问题的结果
            }
        }
        memo[remain] = (min == Integer.MAX_VALUE ? -1 : min); // 如果没有任何一种硬币组合能组成总金额，返回 -1  // 记忆化
        return memo[remain];
    }
    /**
     * 动态规划 完全背包问题
    ...
    public int coinChange8(int[] coins, int amount) {
        ...
        for (int i = 0; i < dp.length; i++) {
            ...
            for (int coin : coins) {
                ...
                dp[i] = Math.min(dp[i], dp[i - coin] + 1); //  dp[i] = min(dp[i - coin] + 1), if i - coin >= 0
            }
        }
        return (dp[amount] == amount + 1) ? -1 : dp[amount];
    }
