...
public class Q707_Design_Linked_List {
     ...
     * 单链表中的节点应该具备两个属性：val 和 next 。val 是当前节点的值，next 是指向下一个节点的指针/引用(next is a pointer/reference to the next node)。
     *
     * 如果是双向链表，则还需要属性 prev 以指示链表中的上一个节点。假设链表中的所有节点下标从 0 开始(Assume all nodes in the linked list are 0-indexed.)。
     *
     * 实现 MyLinkedList 类：
