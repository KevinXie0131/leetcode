...
public class Q83_Remove_Duplicates_from_Sorted_List {
    ...
    public ListNode deleteDuplicates_0(ListNode head) {
        ...
        while(cur.next != null){
            ...
            if(cur.next != nxt){
                cur.next = nxt; // delete elements within two pointers
            }
            cur = cur.next;
        }
        return dummy.next;
    ...
    public ListNode deleteDuplicates_Recursive(ListNode head) {
        ...
        if(head.val == head.next.val){
            ...
            // head = deleteDuplicates(head.next); // works too
        }else{
    ...
    public static ListNode deleteDuplicates_Recursive_func(ListNode head, int preValue) {
        if(head == null){
            return head;
        }
    ...
    /**
     * 递归: 当相邻节点元素值相同时进行重连接操作 - from head to tail 递归
     */
